...
- 참고)
    - 현재 실행되는 프로그램 = 프로세스 
    - 페이징을 배우면 실행되는 모든 프로그램의 모든 데이터와 명령어가 다 메모리에 저장될 필요는 없음
...
컴퓨터로 작업하는 도중에 전원이 꺼져서 작업한 내역을 잃어본 적 있나요? ^^… 전원이 꺼지면 작업한 내역을 잃게 되는 이유는 실행 중인 프로그램들은 메모리에 저장되는데, 메모리는 전원이 꺼지면 저장된 내용이 날아가기 때문이다.
...
- 시스템 버스는 주소 버스, 데이터 버스, 제어 버스로 구성되어 있다.
    - 주소 버스address bus: 주소를 주고받는 통로
    - 데이터 버스data bus: 명령어와 데이터를 주고받는 통로
...
- 인터럽트가 있따면 입출력 작업 동안 CPU는 다른 일을 할 수 있다.


- 순서 (인터럽트 종류룰 막론하고 인터럽트 처리 순서는 대동소이하다.)
...
**✔️인터럽트 요청 신호**
- CPU에게 지금 끼어들어도 되는지 물어보는 것

![img_34.png](img_34.png)


- CPU가 인터럽트 요청을 받아들이려면, 실행 사이클이 끝난 후 플래그 레지스터의 인터럽트 플래그를 확인한다.
- 현재 받아들일 수 있다면 CPU는 받아들이고 인터럽트 처리
- 받아들일 수 없다면 해당 인터럽트 처리하지 않는다.
- 너어어어무 중요하고 긴급하면 인터럽트 플래그로도 막을 수 없다 > NMI(num maskable interrupt. 막을 수 없는 인터럽트)
![img_35.png](img_35.png)
- 받아들이기로 했다면
- 인터럽트 서비스 루틴이라는 "프로그램" 실행
...
- 인터럽트를 보낼 수 있는 주체는 다양하다.
- 키보드, 마우스, 프린터기
- 인터럽트 서비스 루틴의 시작 주소가 각자 다르기 때문에
- 그 주소를 알고 주체를 구분하기 위한 정보 = 인터럽트 벡터
- 인터럽트 벡터: 각각의 인터럽트를 구분하기 위한 정보
![img_37.png](img_37.png)
- 입출력장치는 인터럽트 요청 신호 + 인터럽트 벡터 같이 데이터 버스를 통해 보냄

- CPU가 인터럽트를 처리한다 = 인터럽트 서비스 루틴을 실행하고 본래 수행하던 작업으로 다시 되돌아온다.

- 인터럽트 서비스 루틴 시작 전 기존 작업 정보를 저장해야 함.(백업) > 스택에 백업
